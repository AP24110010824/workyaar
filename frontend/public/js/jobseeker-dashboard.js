...
async function loadProfile() {
  ...
  try {
    ...
    setText("sidebarName", u.full_name || u.name || "Alex Rivera");
    setText("previewName", u.full_name || u.name || "Alex Rivera");
    setText("previewTitle", u.professional_title || u.title || "Senior Designer");
    setText("previewLocation", u.location || u.city || "Remote");
    setText("previewEmail", u.email || "email");

    const avatar = getEl("profileAvatar");
    if (avatar) {
      avatar.innerText = (u.full_name || u.name || "A").trim().charAt(0).toUpperCase();
    }
...
function initDashboardUi() {
  ...
  document.querySelectorAll("[data-section-jump]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      document.querySelector(`.menu-item[data-section="${target}"]`)?.click();
    });
  });
